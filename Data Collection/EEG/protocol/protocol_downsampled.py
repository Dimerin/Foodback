...
SAMPLING_RATE     = 250                 # Hz (must evenly divide native rate)
...
def main():
    ...
    # Check if the desired sampling rate divides the native sampling rate
    native_fs = board_shim.get_sampling_rate(BoardIds.MINDROVE_WIFI_BOARD)
    if native_fs % SAMPLING_RATE != 0:
        print(f"ERROR: {SAMPLING_RATE}Hz must divide native {native_fs}Hz")
        sys.exit(1)
    ...
    try:
        ...
        # Fetch full-rate data
        total_samples = int(native_fs * RECORD_DURATION)
        raw = board_shim.get_current_board_data(total_samples)

        # For each channel, apply anti-alias filter + decimate
        factor = native_fs // SAMPLING_RATE

        dec_data = {}
        for idx, ch in enumerate(plot_thread.exg_chs):
            # scipy.signal.decimate applies a low-pass before downsampling
            dec_data[idx] = decimate(raw[ch], factor, ftype='iir', zero_phase=True)

        # Build down-sampled DataFrame
        N = len(dec_data[0])  # should equal SAMPLING_RATE * RECORD_DURATION
        timestamps = [t0 + i/float(SAMPLING_RATE) for i in range(N)]
        df = pd.DataFrame({'timestamp': timestamps})
        for idx in dec_data:
            df[f'ch{idx}'] = dec_data[idx]
        df['subject']    = subject
